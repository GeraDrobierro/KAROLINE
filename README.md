...
Здесь мы проверяем нахождение слова, которое мы сказали в хеш-таблице, и только после этого выполняет действие. В этом блоке, если я сказа какое-либо значение из хеш-таблице по ключу [‘интернет’], то с помощью webbrowser.open(url) откроется url-ссылка.
